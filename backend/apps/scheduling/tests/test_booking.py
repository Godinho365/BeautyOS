...
def _book(client, prof, svc, hour):
    return client.post(
        "/api/v1/appointments",
        {"customer_name": "Cliente", "professional_id": str(prof.id),
...
def test_agenda_e_calcula_fim_pelo_servico(cenario):
    ...
    r = _book(client, cenario["prof_a"], cenario["svc_a"], 10)
...
def test_rejeita_sobreposicao_mesmo_profissional(cenario):
    client = _client("dono@a.com")
    assert _book(client, cenario["prof_a"], cenario["svc_a"], 10).status_code == 201
    # 10:30 sobrepõe 10:00–11:00 -> conflito
    r = client.post(
        "/api/v1/appointments",
        {"customer_name": "Outro", "professional_id": str(cenario["prof_a"].id),
...
def test_constraint_do_banco_barra_sobreposicao(cenario):
    ...
    a, prof, svc = cenario["a"], cenario["prof_a"], cenario["svc_a"]
    with use_tenant(a.id):
        Appointment.objects.create(
            tenant_id=a.id, customer_name="X", professional_id=prof.id,
            service_id=svc.id, starts_at=_dt(14), ends_at=_dt(15),
        )
        with pytest.raises(IntegrityError):
            with transaction.atomic():  # savepoint: isola o erro esperado
                Appointment.objects.create(
                    tenant_id=a.id, customer_name="Y", professional_id=prof.id,
...
def test_permite_horarios_adjacentes(cenario):
    ...
    assert _book(client, cenario["prof_a"], cenario["svc_a"], 11).status_code == 201
...
def test_rejeita_profissional_de_outro_tenant(cenario):
    ...
    r = client.post(
        "/api/v1/appointments",
        {"customer_name": "X", "professional_id": str(cenario["prof_b"].id),
...
def test_isola_agendamentos_por_tenant(cenario):
    ...
    assert _book(ca, cenario["prof_a"], cenario["svc_a"], 10).status_code == 201
    assert _book(cb, cenario["prof_b"], cenario["svc_b"], 10).status_code == 201
    # Cada dono só enxerga o próprio agendamento.
